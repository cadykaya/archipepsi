...
#
#   tools/style_board.sh [out_dir]
set -e
ROOT="$(cd "$(dirname "$0")/.." && pwd)"
GODOT="${GODOT:-$ROOT/.tools/godot}"
OUT="${1:-$ROOT/docs/art/review/batch002}"
case "$OUT" in /*) ;; *) OUT="$ROOT/$OUT" ;; esac
[ -x "$GODOT" ] || { echo "style_board: no godot at $GODOT" >&2; exit 2; }
...
xvfb-run -a -s "-screen 0 1920x1200x24" "$GODOT" --rendering-driver opengl3 \
  --path "$ROOT/tools/artpreview" -s StyleBoard.gd -- "$ROOT/assets" "$OUT" 2>&1 \
  | grep -E "^\[board\]|SCRIPT ERROR|ERROR: Node" || true
