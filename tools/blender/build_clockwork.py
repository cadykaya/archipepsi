...
def _image(role):
    if role not in _IMAGES:
        ...
        _IMAGES[role] = canvas.to_blender("ck_%s_%s" % (THEME, role))
    return _IMAGES[role]
...
def _wheel(tag, across, thick, at, role="trim_plain", collide="trim",
           upright=False, hub=False):
    """A gear: an eight-sided prism, which IS a toothed silhouette.

    Modelling real teeth costs a box each and buys nothing at 32
    texels/m -- a tooth lands under a texel and the octagon is doing all
    the work anyway.

    `upright` stands the disc up so its face points OUT OF A WALL. This
    argument exists because the first version of this helper built gears
    from two crossed boxes with `rotation_z`, which is the only rotation
    `brushkit.block` offers: a gear lying flat, correct on a vertical
    column shaft and **wrong on a wall**. The manifest said so before any
    render did -- `tp_ck_wall_movement` measured 0.88 m deep on a 0.12 m
    plate, because a 0.62 m square turned 45 degrees in PLAN pokes 0.44 m
    straight through the wall it is supposed to be mounted on.

    `brushkit.spin` turns the geometry about its own bbox centre, which
    is the one safe way to rotate a part that is already positioned.
    """
    obj = brushkit.prism(tag, across / 2.0, thick, 8, at)
    if upright:
        brushkit.spin(obj, "x", 90.0)
    made = [_paint(obj, role, collide)]
    if hub:
        # A raised boss at the centre. Without it an octagon face is a
        # plate, and `CK_approach` showed exactly that: the wheels read
        # as slabs until something said where the arbor goes. One prism,
        # 28 triangles, and it is the difference between a gear and a
        # stop sign.
        boss = brushkit.prism("%s_hub" % tag, across * 0.11, thick * 2.0,
                              8, at)
        if upright:
            brushkit.spin(boss, "x", 90.0)
        made.append(_paint(boss, "wall", collide))
    return made
...
def gear_column():
    ...
    """
    h = 3.6
    body = _b("ck_col_shaft", (0.34, 0.34, h), (0.0, 0.0, h / 2.0), "wall")
    parts = [
        ...
        _b("ck_col_plinth", (0.72, 0.72, 0.22), (0.0, 0.0, 0.11),
           "trim_plain", "trim"),
        _b("ck_col_bearing", (0.46, 0.46, 0.14), (0.0, 0.0, 3.53),
           "trim_plain", "trim"),
        _b("ck_col_collar", (0.44, 0.44, 0.10), (0.0, 0.0, 1.12),
           "trim_plain", "trim"),
    ]
    parts += _wheel("ck_col_wheel", 1.10, 0.10, (0.0, -0.16, 2.20),
                    "trim_plain", "trim", upright=True, hub=True)
    parts += _wheel("ck_col_pinion", 0.44, 0.09, (0.62, -0.16, 1.72),
                    "trim_plain", "trim", upright=True, hub=True)
    return body, parts
...
def wall_movement():
    ...
    """
    body = _b("ck_mv_plate_l", (0.90, 0.12, 3.0), (-0.63, 0.0, 1.5), "wall")
    parts = [
        _b("ck_mv_plate_r", (0.90, 0.12, 3.0), (0.63, 0.0, 1.5), "wall"),
        ...
        # these.
        _b("ck_mv_rail_top", (2.16, 0.12, 0.22), (0.0, 0.0, 2.89), "trim_plain", "trim"),
        _b("ck_mv_rail_low", (2.16, 0.12, 0.22), (0.0, 0.0, 0.11), "trim_plain", "trim"),
    ]
    parts += _wheel("ck_mv_gear_big", 0.62, 0.10, (-0.10, -0.11, 1.95),
                    "trim_plain", "trim", upright=True, hub=True)
    parts += _wheel("ck_mv_gear_small", 0.36, 0.09, (0.40, -0.11, 1.62),
                    "trim_plain", "trim", upright=True, hub=True)
    ...
    parts.append(_b("ck_mv_bridge", (0.78, 0.07, 0.50), (0.12, -0.12, 1.75),
                    "trim_plain", "trim"))
    return body, parts
...
def pendulum_lamp():
    ...
    """
    body = _b("ck_pd_bracket", (0.34, 0.30, 0.14), (0.0, 0.0, 0.93), "trim_plain", "trim")
    parts = [
        _b("ck_pd_cap", (0.20, 0.20, 0.07), (0.0, -0.10, 0.865), "trim_plain", "trim"),
        _b("ck_pd_rod", (0.05, 0.05, 0.60), (0.0, -0.10, 0.54), "trim_plain", "trim"),
    ]
    ...
    parts += _wheel("ck_pd_bob", 0.26, 0.10, (0.0, -0.10, 0.16),
...
def key_escutcheon():
    ...
    """
    body = _b("ck_ky_plate", (0.46, 0.08, 0.54), (0.0, 0.0, 0.27), "wall")
    parts = [
        _b("ck_ky_bezel", (0.34, 0.07, 0.34), (0.0, -0.075, 0.32), "trim_plain", "trim"),
        _b("ck_ky_dial", (0.22, 0.09, 0.22), (0.0, -0.085, 0.32),
           "trim_plain", "trim"),
        _b("ck_ky_arbor", (0.10, 0.10, 0.10), (0.0, -0.115, 0.32), "trim_plain", "trim"),
        _b("ck_ky_spoke", (0.24, 0.05, 0.05), (0.0, -0.125, 0.32),
           "trim_plain", "trim"),
    ]
    return body, parts
...
def fallen_hand():
    ...
    thinking about it.
    """
    body = _b("ck_hd_shaft", (1.20, 0.10, 0.07), (0.0, 0.0, 0.035),
              "trim_plain", "trim", rotation_z=8.0)
    parts = [
        _b("ck_hd_boss", (0.16, 0.16, 0.11), (-0.60, -0.04, 0.055), "trim_plain", "trim"),
        # The buckle. Two segments meeting at an angle is the whole
        # difference between a fallen hand and a length of bar stock.
        _b("ck_hd_bend", (0.30, 0.09, 0.06), (0.24, -0.06, 0.03),
           "trim_plain", "trim", rotation_z=24.0),
        _b("ck_hd_spade", (0.32, 0.20, 0.07), (0.50, -0.11, 0.035),
           "trim_plain", "trim", rotation_z=20.0),
        _b("ck_hd_tip", (0.28, 0.07, 0.05), (0.78, -0.18, 0.025),
           "trim_plain", "trim", rotation_z=20.0),
    ]
    parts += _wheel("ck_hd_tail", 0.28, 0.09, (-0.60, -0.04, 0.045),
                    "trim_plain", "trim")
    return body, parts
...
def door_bezel():
    ...
    """
    jamb = 0.34
    body = _b("ck_dr_lintel", (DOOR_W + jamb * 2.0, 0.30, 0.34),
              (0.0, 0.0, DOOR_H + 0.17), "trim_plain", "trim")
    parts = []
    for sign, tag in ((-1.0, "l"), (1.0, "r")):
        x = sign * (DOOR_W / 2.0 + jamb / 2.0)
        parts.append(_b("ck_dr_jamb_%s" % tag, (jamb, 0.30, DOOR_H),
                        (x, 0.0, DOOR_H / 2.0)))
        parts.append(_b("ck_dr_marks_%s" % tag, (jamb, 0.44, 2.20),
    ...
    parts.append(_b("ck_dr_crown", (0.80, 0.46, 0.24),
                    (0.0, 0.0, DOOR_H + 0.17), "trim_plain", "trim"))
    return body, parts
...
def main():
    ...
    for name, build, checks in ASSETS:
        ...
        made[name] = entry
        print("[clock] %-24s %4d tris, %d part(s)"
    ...
    with open(path, "w", encoding="utf-8") as handle:
        json.dump(made, handle, indent=1, sort_keys=True)
        handle.write("\n")
    print("[clock] manifest %s" % os.path.relpath(path, common.REPO_ROOT))
