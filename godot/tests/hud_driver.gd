...
func _archive_provenance() -> void:
	...
	_check(_count_containing(labels, "read: magic / green / capacity") == 1,
			"the concepts Epsilon read are on the row")
	_check(_count_containing(labels, "read: capacity / shard") == 1,
			"the upgrade interpretation shows its own concepts")
