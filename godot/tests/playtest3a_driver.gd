...
func _test_an_authored_offer_changes_navigation_in_a_generated_zone() -> void:
	...
	# is one call.
	# THE LANE'S PARENT IS THE RAIL'S FRAME, not the lane.
	# `AffordanceNodes.Volume._room_to_world` hands `offer_rail` the
	# PARENT's `global_transform`, so reading the curve through the
	# lane's own transform is off by the lane's local offset -- which is
	# how a body placed at "the rail's start" stood beside it.
	var frame: Transform3D = (lane.get_parent() as Node3D).global_transform \
			if lane.get_parent() is Node3D else Transform3D.IDENTITY
	# START INSIDE THE VOLUME. `AffordanceNodes.Volume` is what calls
	# `offer_rail`, on body-entered -- so a body placed at the rail's own
	# first point is a body the rail is never offered to. The lane's
	# origin is inside its own trigger by construction.
	var start: Vector3 = lane.global_position
	# AND THE RAIL'S DIRECTION WHERE THE BODY IS, not the lane's +Z.
	# Those agree only when the rail runs along the lane's forward axis,
	# which is a property of one rail rather than of rails. `catch`
	# refuses a body whose velocity is not along the tangent, so a rail
	# that climbs or turns was being pushed at right angles to itself.
	var into_room := frame.affine_inverse()
	var here := rail.nearest_offset(into_room * start)
	var along: Vector3 = (frame.basis * rail.tangent(here)).normalized()
