...
def reading_errors(interpretation, request) -> list[str]:
    ...
    One check, and deliberately only one: an Echo with no concepts breaks
    the chain outright — the archive shows "read this as:" followed by
    nothing, and the player has no way to argue with the mechanics that
    followed.

    A second check briefly lived here, refusing a reading that shared no
    vocabulary with the item, on the theory that it would catch concepts
    pasted from another Echo. It was wrong in both directions: it passed
    any reading containing a substring of "artifact"/"elsewhere"/
    "borrowed", and it refused "Master Sword reads as obligation" — the
    exact kind of reading §15 argues FOR ("the best interpretation is not
    the one most similar to the source item"). Attachment is not decidable
    from an item name. See `concepts.shares_vocabulary_with`.

    The mode is not checked against the operations either. A provider that
    reached for something systemic and mislabelled it has made a
    describable mistake, and forcing a relabel is the repair loop's job
    only if the label is what is wrong. Left to the prompt, which states
    the rule, and to `mode_for_operations` for the deterministic
