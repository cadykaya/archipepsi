# Archipepsi — APWorld Specification (v0.4)
...
| `Tier 1` | Checks 011–020 | `state.has("Pepsi Key", player, 1)` |
| `Tier 2` | Checks 021–030 | `state.has("Pepsi Key", player, 2)` |

`Menu → Start → Tier 1 → Tier 2`, chained.

The bridge mirrors this tier structure when deciding which unchecked locations are eligible for generated content. The mirror lives in `schemas/constants.py`; do not re-derive it in two places.
...
```
Pepsi Key      -> 89_200_001   progression   x2
...
**Pepsi Key** — monotonic, never spent. 0 keys: Start eligible. 1: +Tier 1. 2: all 30.
...
So the generator's completion condition is effectively "2 Pepsi Keys".

**Runtime completion is deliberately stricter.** The client reports goal (`ClientStatus.CLIENT_GOAL`, value 30) only when **Check 030 is confirmed**, and Check 030 is only reachable through the finale Zone, which requires 2 Keys **and** 24 of the other 29 Checks (`DESIGN.md` §10.6).
...
This forces Epsilon Coins outside the Archipepsi world wherever legal placement exists. Pepsi Keys are **not** forced non-local; normal fill decides.

## 7.1 Solo testing variant

For testing Archipepsi alone, or for a demo that must not depend on other players' pace:
...
The default YAML makes the goal depend on other players finding two Pepsi Keys, which is correct Archipelago citizenship and bad demo determinism. Keep both files. Mock Campaign covers the same need without a server at all.
...
- slot data is schema version 4 and contains no placements
