# Archipepsi — Implementation Plan (v0.4)
...
9. Generate a real solo seed. Then generate a multiworld seed with a second world and `non_local_items: Epsilon Coin`.
10. APWorld self-checks from `APWORLD_SPEC.md` §9.

**Milestone: `Archipepsi` generates real seeds. Fully verifiable, no engine involved.**
...
15. Reconstruct `items_received`; derive Pepsi Keys, Coins, Static.
...
20. A **CLI smoke test** that drives the whole loop with no Godot: connect → scout → allocate → fallback-generate → claim → confirm → echo → snapshot.
...
### Phase 3 — Godot vertical slice (~90 min)
...
25. First-person controller using the binding constants. LMB Pepsi Pop, RMB Echo.
...
35. Fallback Zone and Echo generators (validated through the same validator).
...
Fill 007–030 deterministically with a mix of foreign items and the native pool, ensuring at least 2 Pepsi Keys and several Coins arrive.
...
Do not search for, download, browse, or evaluate external asset packs, texture packs, or model libraries. Flat materials and procedurally generated 16×16 textures. See `DESIGN.md` §20.
...
**Core** — repo starts from documented commands; the APWorld generates; the bridge connects to a real server; all 30 locations scout; the Hub loads; the player moves, jumps, and fires Pepsi Pop; corridor and arena build; a Check can be claimed and confirmed; pending-check and save logic work; a Conference Call Echo can be acquired, equipped and fired; reload preserves Zone and Echo.
