# Archipepsi — Acceptance Tests (v0.4)
...
`schemas/test_schemas.py` — 37 tests at time of writing, all passing. Run them first, before writing anything else. The count will grow; what matters is that they are green on arrival, so any red one is a regression you introduced.
...
25. With 0 Pepsi Keys, only Checks 001–010 are eligible.
26. Receiving one Pepsi Key makes 011–020 eligible.
...
30. `WAITING_FOR_AP` clears automatically when a Pepsi Key arrives.
...
38. Pool contains 2 Pepsi Keys, 10 Epsilon Coins, 18 Epsilon Static.
...
45. Slot data is schema version 4 and contains no location→item placements.
...
Clear every eligible Tier-0 Check with no Pepsi Key delivered.
Expect: unsold shop reservations release; the Hub shows `WAITING FOR ARCHIPELAGO`; the portal is disabled but Hub, inventory and shop remain usable; delivering a Pepsi Key restores `ZONE_AVAILABLE` without a restart.
...
Expect: the Hub shows `FINALE_AVAILABLE` with progress; the portal generates a single-Check Zone containing only Check 030; confirming it reports goal once; the Hub becomes `CAMPAIGN_COMPLETE`.
...
- movement feels good enough to jump a 3.0 m gap reliably
