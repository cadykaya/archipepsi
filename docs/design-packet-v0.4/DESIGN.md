...
## 3.4 Ugly is a feature

Low-resolution procedural textures, primitive geometry, flat materials, simple lighting and enemies. No AI-generated art, models, shaders or audio.
...
**Native Item** — a real Archipepsi AP item: Pepsi Key, Epsilon Coin, Epsilon Static.
...
| Mouse | look |
| Space | jump |
| **LMB** | **Pepsi Pop — always, never replaced** |
...
**v0.4 decision.** v0.3 bound the equipped Echo to left mouse while also promising every mandatory encounter was beatable with Pepsi Pop. Those cannot both be true: equipping an active Echo removed the weapon the guarantee rested on, so the promise silently became "beatable, provided the player realises they should unequip" — and a 25-damage/15-second-cooldown Echo is legal under the bounds and strictly worse than Pepsi Pop in a `kill_all` room.

Splitting the bindings makes the guarantee unconditional and needs no player understanding. It is also less code: no conditional rebinding on equip. And it is the only version where you can fire the Conference Call, get launched backwards, and keep Pepsi-popping on the way down.

---

# 8. Base player capability

Without any Echo the player can walk, jump, interact, and attack with **Pepsi Pop**: simple hitscan, low damage, short cooldown, unlimited ammo, no generated data. Every mandatory encounter is beatable with it.

Movement and combat constants are in `schemas/constants.py` and are **binding**. `SAFE_BASE_JUMP_GAP` is *derived* from the jump arc:

```
apex   = JUMP_VELOCITY² / (2·GRAVITY)   = 1.33 m
airtime= 2·JUMP_VELOCITY / GRAVITY      = 0.67 s
reach  = WALK_SPEED · airtime           = 4.67 m
SAFE_BASE_JUMP_GAP = reach × 0.64       = 3.0 m
```

v0.3 required the game to *measure* this in-engine. It is now computed, unit-tested, and recomputes correctly if movement is retuned — which is what makes "every mandatory path is completable with base movement" a checkable claim rather than a hope.

Enemy stats are chosen so the worst legal Zone (14 enemies including one brute) is ~25 seconds of sustained Pepsi Pop fire. Asserted in `test_schemas.py`.

Epsilon should prefer making Echo weapons much more satisfying than Pepsi Pop; the bounds allow roughly 2.6× its DPS.
...
- it is in an unlocked tier (tier N requires N Pepsi Keys)
...
1. Compute unlocked tiers from received Pepsi Keys.
...
- the player has received **2 Pepsi Keys**, and
...
**This is deliberately stricter than the APWorld's own completion condition,** which only requires 2 Pepsi Keys. A runtime stricter than AP logic is safe — the seed stays beatable and the solver's assumption still holds; it just takes longer. The dangerous direction is the reverse.
...
1. Verify the location is still server-missing and the balance suffices.
2. Persist a `PendingCheck` with `source: "shop"` and `shop_cost`.
3. Add the cost to persisted `coins_spent`.
4. **Save.**
5. Send the location check.
6. On server confirmation: finalize, release the reservation, create the Echo, clear the pending record, save.

If the connection drops after step 4, coins stay spent and the check is re-driven on reconnect.

**Rollback.** v0.3 rolled back when "the server permanently rejects the location." Archipelago sends no such rejection — invalid IDs are silently ignored, and `CommonContext.check_locations()` filters them out before sending. The real, locally-checkable trigger is: after a full snapshot, the location ID is absent from `missing_locations ∪ checked_locations`, i.e. it is not this slot's location at all. Then roll back once, subtract the cost, release the reservation, and log.
...
coins_spent     = persisted sum of confirmed + pending shop costs
coins_available = max(0, coins_received - coins_spent)
```

Never store `coins_available`. Never increment from a callback. Always recompute from the reconstructed list.
...
| `WAITING_FOR_AP` | disabled | nothing eligible, campaign not finished |
| `CAMPAIGN_COMPLETE` | disabled | goal sent |

`HubStatus.finale_available` is a **separate boolean**, independent of `mode`.

This matters: the finale unlocks at 24 of 29 Checks, so up to 5 ordinary Checks normally remain when it appears. If the finale were a mode that displaced `ZONE_AVAILABLE`, those Checks would become permanently unreachable content — the player would be forced to end the campaign the moment they qualified.

So when both are possible, the Hub offers **both**, and the player chooses: keep playing, or go finish it. `RequestNextZone.finale` carries the choice. `FINALE_ONLY` is the narrower case where nothing ordinary is left.

`WAITING_FOR_AP` and an available finale are mutually exclusive by construction — if you can start the finale, you are not waiting on anyone.
...
Reached when there are zero eligible non-goal Checks and the finale is not unlocked — after unsold shop reservations have been released.

The Hub, inventory and shop remain fully usable. The portal is disabled, not broken. The state clears automatically when AP state changes (a Pepsi Key arrives, or a location's status changes).

This is the most likely state a real six-player run will hit, and it must read as intentional rather than as an error. The status board should say plainly that other players hold your progression — that is normal Archipelago interdependence, and it is the moment the game is most obviously *part of a multiworld*.
...
- **resets** transient state: living and dead enemies, partial enemy HP, unsatisfied objective progress, player HP

The Zone stays `ACTIVE`; no new Zone can be generated. Entering the Hub portal resumes the same Zone.

## 14.3 Finishing

The exit portal unlocks when every Check assigned to the Zone is server-confirmed. Using it:

1. shows a short Zone summary
2. returns the player to the Hub
3. appends the summary to campaign history
4. clears `active_zone_id`, moves the record to `COMPLETE`
5. advances the Track cursor, increments the completed-Zone count
6. applies shop cadence
7. saves

**A Zone with every Check confirmed completes automatically**, wherever the player is. If they are inside it, the exit portal unlocks and the summary shows when they use it. If they are in the Hub — because they left early, or because reconnect reconciliation confirmed the last Check — the record moves to `COMPLETE` immediately and the Hub returns to `ZONE_AVAILABLE`.

The exit portal is how you *travel*, not how a Zone is marked done. Requiring a walk back into a finished Zone to formally end it would be busywork, and it would let a player strand themselves with an `ACTIVE` Zone that has nothing left to do and blocks new generation.
...
- **`primary`** — exactly one initiator effect plus 0–2 modifiers; `cooldown` required; fired with RMB.
- **`passive`** — 1–2 passive effects; no `cooldown` field exists; RMB does nothing.

Modifiers (`recoil_self`, `knockback_target`) require a damage initiator in the same Echo. v0.3 permitted `knockback_target` alone and `recoil_self` on a heal; both are now unrepresentable.
...
**Native AP progression** — Pepsi Keys, understood by AP logic, determine tiers. 0 keys: Checks 001–010. 1 key: +011–020. 2 keys: +021–030.
...
Mandatory routes use base movement only. Mandatory combat is always completable with Pepsi Pop. Owned Echoes strongly influence design and may provide optional shortcuts, combat advantages, faster traversal, and secrets.
...
Low-poly, blocky, deliberately crude, readable, colorful by theme. Nearest-neighbour filtering. Primitive Godot meshes and repeated materials. Not a voxel engine.
...
Optional. If included: footsteps, Pepsi Pop, hit, reward, **Echo acquired**, shop purchase. Generate simple tones procedurally rather than sourcing files. Never block core completion on audio — except that the Echo acquired sound materially improves §16 and is worth the five minutes.
...
`epsilon_creativity` (0 Conservative / 1 Playful / 2 Unhinged, default 1) is persisted in the campaign save, so a reloaded campaign remembers it was Unhinged. It changes model instructions only — never schemas, bounds, or validation.
