# Archipepsi — Epsilon Contract (v0.4)
...
A theme is a validated palette, not generated art: `grass_block`, `stone_dungeon`, `neon_city`, `gothic_castle`, `desert_scrap`, `void_glitch`.

Each defines floor / wall / accent colors, a sky/background color, and optional light color and energy. **Textures are generated procedurally in code** (`Image.create()`, 16×16, nearest-neighbour). No image files, and no searching for asset packs — see `DESIGN.md` §20.

Suggested mapping: Mario→`grass_block`, Ocarina→`stone_dungeon`, Bomb Rush→`neon_city`, Dark Souls→`gothic_castle`, Borderlands→`desert_scrap`, Archipepsi-native→`void_glitch`. Epsilon may deviate.
...
Stats are in `schemas/constants.py` and are binding. They are chosen so the worst legal Zone is roughly 25 seconds of sustained Pepsi Pop fire — bounded on purpose, since the limits alone would otherwise permit a four-minute plinkfest.
...
```json
{
  ...
  "campaign": {
    ...
    "completed_zone_summaries": [
      {"name": "Neon Drain", "theme": "neon_city", "target_game": "Bomb Rush Cyberfunk"}
    ]
  },
  "player": {
    "pepsi_keys": 1,
  ...
  "catalog": {
    "themes": ["grass_block", "stone_dungeon", "neon_city", "gothic_castle", "desert_scrap", "void_glitch"],
...
```json
{
  ...
  "player_state": {"existing_echoes": [], "pepsi_keys": 0, "coins_available": 2},
...
But: **every mandatory path and objective must remain completable with base movement and Pepsi Pop.** `featured_echo_ids` is descriptive metadata only. There are no Echo-only gates.
