...
| **C2** | A signal-driven actuator generalised from `PoweredLink` | **done** (OV04 P15 + P13) | `godot-actuator`, `godot-constraints` | `Actuator` is that generalisation: §21.1's transition table and §21.1.1's power-loss table over all twelve kinds, nine kinematic and three constraint-driven. `RailSpan`/`AlignmentControl` and `ShuttleDeck`/`CallLever` remain their own concrete chains and keep their motion curves; what is shared is the contract. The signal GRAPH that would drive it is P14, which DESS-09 measured as not ready — one chain, no node vocabulary, and a framework no room uses is what §P14.5 warns against |
...
| **C4a** | `ServiceShutter`'s blocked-closure behaviour against §21.2 | **done** (OV04 P15) | `godot-actuator` | The panel now stops, **reverses to fully open**, and retries every 1.0 s for as long as the doorway is occupied, per `01_RELIABLE_CORE.md:2318`. The rule lives in `SafeClosure`, shared with `Actuator`; the protected set widened from the player to §21.2's "player or any `required = true` object". Reverting the reversal produces nine failures |
