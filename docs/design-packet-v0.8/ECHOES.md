...
**Echoes 2.0.** This document is normative for everything an Echo is, and
supersedes DESIGN §15/§19 and EPSILON_SPEC §8/§10/§12.2/§13 as they stood
in v0.7. It specifies `schemas/echo.py` at `SCHEMA_VERSION = 8`; the
executable version of this contract lands in **S1** and is binding from
that moment (see §19, §20).
...
> **Packet status.** `schemas/` in this packet is still the v7 contract and
> remains binding for everything Echoes 2.0 does not touch. This document
> is normative for the v8 Echo contract; **S1 lands the executable
> version**, at which point `schemas/echo.py` becomes binding over this
> prose in the usual authority order.
