...
**v0.8.** The fallback keeps this pinned shape and stays deliberately boring: it emits a single `CREATE` operation contributing one Action or one Trait, mode `literal`, no resources, no rules, no links, no merges, no affordances. Its job is to be provable, not interesting. That also means the fallback can never breach a budget, can never dangle a target, and can never be the reason a fold fails — which is exactly what makes it usable as the test oracle for every stage that follows.

The **mock** provider is the one that must grow: `--epsilon=mock` has to exercise resources, rules, links, merges and the wider action catalog, or the headless integration run stops proving anything about the systems S2–S6 add. That growth is scheduled as part of S10.
