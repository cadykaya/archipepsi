# Archipepsi — Acceptance Tests (v0.8)
...
# 5.7 Echoes 2.0 — invariant tests

One test per invariant in `ECHOES.md` §18. These are written **with the
stage that introduces the invariant**, not retrofitted afterwards; a stage is
not done until its rows here are green.

| | Invariant | Test | Stage |
|---|---|---|---|
| **I1** | AP truth untouched | No component schema has a location, item or check field beyond read-only provenance. Structural — asserted over the exported JSON Schema, so it cannot rot. | S1 |
| **I2** | No code executed | Every generated reference resolves through an allowlist; `extra="forbid"` rejects invented fields. A recipe naming an unknown primitive, event, condition, effect or status is rejected, not dropped. | S1 |
| **I3** | Movement floor | Over a randomised sweep of legal trait stacks, every derived movement stat is ≥ base, and `max_safe_gap` is unchanged from its v0.7 value. | S5 |
| **I4** | Mandatory-path independence | No mandatory path element resolves to a non-base capability, across a seeded Zone sweep. | S9 |
| **I5** | Termination | An effect that fills a resource does **not** dispatch `resource_full` in the same tick; it dispatches on the next one, once, on the edge. A rule that fills on empty and drains on full oscillates at the cooldown rate and never exceeds the per-tick cap. | S4 |
| **I6** | Fold determinism | Folding a log twice yields identical mechanics. A log whose `interpretation_seq` order **differs** from `source_location_id` order folds by sequence — the regression case is an interpretation targeting a component created by a higher-numbered location. | S1 |
| **I7** | Curse ceiling | Every component carrying a severe downside declares `requires_equipped`; no always-on component exceeds the mild bound. | S5 |
| **I8** | Contextual budgets | A `CREATE` that would breach a hard budget is rejected and triggers repair; the repaired interpretation upgrades, links or merges instead. | S4 |
| **I9** | Persistence split | Definitions, maxima and upgrades survive a save/load; current values and statuses reset on Zone entry. Quit, reload and re-enter loses no capability and preserves no mana. | S3 |
| **I10** | Alias soundness | Self-merge rejected; cycle rejected; merging into an absorbed id rejected; an absorbed id still resolves after two further merges; provenance from both sides survives. | S6 |
| **I11** | Target liveness | An operation naming a component that does not exist at its sequence position fails the fold **loudly**. Asserted as a raise, not a warning — a skipped operation is a silently different build. | S1 |
| **I12** | Affordance capability | A Zone containing a `water_volume` is rejected when the campaign owns no water capability; accepted when it does. Evaluated over **owned**, not equipped. | S9 |
| **I13** | Local rewards | No local reward is an AP item, location, Check, Coin, Key or Echo, and none sits on a mandatory path. | S9 |
| **I14** | Blink validity | `blink` never resolves without a surface hit, never exceeds its bound, and never lands inside geometry or outside Zone bounds. Swept across every chamber builder. | S2 |

Two more that belong to the migration rather than to an invariant:

- **S1-M1 — a v7 save corpus loads, folds and plays.** Every campaign in the
  corpus migrates, and the migrated log's fold reproduces the loadout the v7
  save described.
- **S1-M2 — `make replay` accepts both archive versions.** A v7 generation
  archive still re-validates after the schema bump, or the benchmark corpus
  is worthless.
...
> **v0.8.** Extended to affordances: a Zone generated for a campaign owning
> `grapple_anchor` may contain anchors, and the run must still complete with
> base movement and Static Pulse alone with every Action slot empty.
