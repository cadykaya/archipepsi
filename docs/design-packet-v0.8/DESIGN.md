...
v0.8 keeps this and makes it structural: the campaign stores an **append-only log of interpretations**, and the live mechanical state is a pure fold over it (`ECHOES.md` §2.1). Nothing is ever removed from the log, so no capability can be lost, and provenance can never be rewritten. Growth happens by appending an interpretation that upgrades, modifies, links or merges what is already there — never by editing history.
...
**Echo** — a permanent local-only *interpretation* of a real item the Archipepsi player sent to another player. Not an AP item. Keyed by `source_location_id`, ordered by `interpretation_seq`.

**Component** — what an Echo actually contributes: an Action, Trait, Resource, Rule, Status, Affordance or Info readout. One Echo may contribute several. Only Actions occupy a slot (`ECHOES.md` §2).

**Local reward** — a payoff that is not Archipelago's: an Epsilon note, a challenge marker, a cosmetic, a Hub decoration, a Lab fixture, an archive entry. Never an AP item, location, Check, Coin, Key or Echo (`ECHOES.md` §14.2).
...
| Q / wheel | cycle within the highlighted slot |
| Tab | Echo archive |
...
**v0.8.** The single Echo slot becomes four categorised slots. The guarantee is unaffected — LMB is still Static Pulse, still never replaced, and a global `damage_dealt` trait multiplying its damage is not a replacement. Epsilon assigns a slot *category* to each Action it creates; it never chooses a keybind, and the grammar above is fixed by the game, not by generation (`ECHOES.md` §9).
...
## 15.2 What an Echo contributes

**`ECHOES.md` is normative for everything in this subsection.** Summarised here because it changes the shape of the game:

An Echo is an *interpretation* that contributes one or more **components** — Action, Trait, Resource, Rule, Status, Affordance, Info. Only **Actions** occupy a slot. Traits, resources, rules, affordances and readouts are simply true once owned, so a Check keeps mattering for the rest of the run whether or not you ever equip it.

An interpretation does not have to invent something new. It may `CREATE`, or `UPGRADE` / `MODIFY` / `LINK` / `MERGE` what you already own — which is where build evolution comes from, and why *Longshot* after *Hookshot* is one grapple at Mk II rather than a second grapple.

Four Action slots (Echo A, Echo B, Mobility, Utility) plus favourites. Rarity, weight and fusion remain out of scope.

## 15.3 Shape

Defined by `schemas/echo.py` at `SCHEMA_VERSION = 8`, specified by `ECHOES.md`. An interpretation carries its source identity, the concepts Epsilon read it as, a declared interpretation mode, and 1–4 **operations**. Components are drawn from closed, validated catalogs; the effect and action vocabularies are enumerations, never free text, and nothing generated is ever executed.

**Trait bounds are still set by the traversal guarantee, not independently, and v0.8 makes the rule blunter than v0.7's.** The derived movement stats — `move_speed`, `jump_height`, `gravity`, `air_control` — have a **hard floor at base**: no combination of owned components may leave you worse at clearing a gap than the base kit. `max_safe_gap` and every generated jump therefore stay valid completely unmodified.

That is a strictly stronger statement than v0.7's per-Echo caps, and it has to be, because v0.8 traits are always-on and stack across everything you own rather than applying one at a time. Downside expresses in channels that bite without blocking — `ground_friction`, `damage_taken`, resource drain, visibility.
...
A scrollable list showing name, source game, source item recipient, **source location**, description, what the interpretation contributed, and a concise effect summary. Showing the source location matters because two Hookshot Echoes must read as *Check 002's* and *Check 026's* rather than as a confusing duplicate.

v0.8 adds three things the archive has to carry:

- **Provenance chains.** A component upgraded three times shows every AP item responsible, in order — "Mk I ← Hookshot (Ocarina of Time), Mk II ← Longshot…". Provenance is never deleted or rewritten.
- **The concepts Epsilon read.** "water / buoyancy / pressure" is half the charm and is stored on the interpretation for exactly this.
- **Slotting.** The player assigns owned Actions to the four categorised slots, favourites them, and compares them. Traits, resources, rules and readouts are listed but not slottable, because they are always on.
...
There are **no** Echo-gated mandatory gates. The validator can verify Echo *ownership* but cannot prove generated geometry is *solvable* with a grapple or a gravity modifier — and that distinction is why the rule exists. The Zone schema has no field capable of expressing a mandatory Echo requirement, so this is structural rather than a rule someone has to remember.

Hard gates return only after a mechanical reachability validator or much more constrained gate templates exist.

**v0.8 widens what "optional" can contain, and tightens the rule to match.** Owned mechanics now reach the generator as a vocabulary: an unlocked affordance lets Epsilon place grapple anchors, breakable walls, water volumes, rails, wind volumes and bounce pads. Three constraints keep that safe, all enforced by the Zone validator rather than by good intentions:

1. **Never mandatory.** An affordance feature may not lie on the mandatory path, and may not host an AP reward, an exit or an objective.
2. **Never unusable.** A feature may only appear if the player's *owned* mechanics can actually interact with it (`ECHOES.md` §13.1). A water volume in a run with no way to move through water is set dressing that looks like content, which is worse than nothing. Capability is evaluated over owned components, not equipped ones — you can always slot what you own.
3. **Never Archipelago's.** What optional content pays out is drawn from the local-reward catalog (`ECHOES.md` §14.2): notes, challenge markers, cosmetics, Hub decorations, Lab fixtures, archive entries. Never an AP item, location, Check, Coin, Key or Echo.

The inventory expands the level generator. It never creates a randomizer softlock.
