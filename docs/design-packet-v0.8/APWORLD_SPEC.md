# Archipepsi — APWorld Specification (v0.8)

> **Unchanged from v0.7.** Echoes 2.0 does not touch the Archipelago side:
> the same 30 locations, 3 native items, 3 tiers, Victory event and slot
> data. An Echo has never been an AP item and still is not one — v0.8 only
> changes what an Echo means *locally*. Version-stamped for the packet
> roll-forward; otherwise the v0.7 contract verbatim.
