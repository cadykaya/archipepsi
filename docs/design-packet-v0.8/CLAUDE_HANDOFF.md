# Archipepsi — Coding Agent Handoff (v0.8)
...
`README.md`, then `DESIGN.md`, `TECHNICAL_ARCHITECTURE.md`, `APWORLD_SPEC.md`, `EPSILON_SPEC.md`, **`ECHOES.md`**, `IMPLEMENTATION_PLAN.md`, `ACCEPTANCE_TESTS.md`.
...
`../design-packet/` (v0.3), `../design-packet-v0.4/`, `../design-packet-v0.7/` and `../audit/` are history. Do not implement from them.

---

## Mission

**The POC (Phases 0–7) is built and green.** v0.8's work is Echoes 2.0: the ten stages in `IMPLEMENTATION_PLAN.md` §2.5, specified by `ECHOES.md`. Work autonomously for as long as the session permits.

Two things about that document specifically. It **supersedes** the v0.7 Echo contract rather than extending it — do not try to keep both alive. And it is deliberately staged so S1 changes no mechanics at all: if S1 makes the game play differently, S1 is wrong.
...
- Set `SKIP_REQUIREMENTS_UPDATE=1` **before** importing `CommonClient`, or the import runs pip and blocks on `input()`.
- `on_package` is synchronous. Schedule async work with `create_task` and keep the reference.
- `CommonContext` clears `items_received` on disconnect — keep your own last-known copy rather than recounting into zeros.
- Use `ctx.slot_concerns_self()`, not `player == slot`.
- Set `ctx.finished_game` alongside your own `goal_sent`.
...
- **Campaign state is replaced, never edited.** `schemas/` models are frozen and every collection is a tuple; all persistent change goes through `schemas/transitions.py`, which builds the complete next `CampaignSave` and validates it in one step. Never `model_copy(update=...)`. Never split a multi-field change into two assignments — completion and abandon are single transitions because the intermediate state is illegal by design. This replaced `validate_assignment=True`, which only ever covered top-level assignment and let `save.zones["z1"].state = "COMPLETE"` write a save that would not load back. See `TECHNICAL_ARCHITECTURE.md` §7.0.
- **Read derived Hub fields off the snapshot.** `portal_enabled`, `finale_unlocked`, `finale_offered`, `accepts_zone_request`, `generation_in_progress` and `coins_available` are computed and exported into `protocol.schema.json`. Re-deriving any of them in GDScript is the drift `constants.gd` exists to prevent.
- **Connectivity is not campaign state.** An Archipelago dropout sets `ap_online=False` and leaves `hub.mode` alone.
...
**Copy `schemas/` verbatim.** It runs and it is tested. Run `pytest` on it before writing anything else — 110 tests, all green, both standalone and from the repo root. Do not retype the models from the prose; the prose describes them, the code *is* them. Regenerate `constants.gd` and the JSON Schemas with `python schemas/export.py` and never hand-edit the outputs.
...
**Do not go asset shopping.** No searching for, downloading, or evaluating texture packs, asset packs or model libraries. No Blender, even though it is installed. Materials and 64×64 procedural textures written in code. This rule exists because that search is an easy way to lose 45 minutes.

**The look is 1998, not Minecraft.** GoldSrc/Quake-era brushwork: chunky low-poly prisms, wedges and ramps; boxy industrial rooms, vents, catwalks, pipes; harsh simple lighting; grimy low-res procedural textures. Not giant cubes, not voxels, not a visible grid. See `DESIGN.md` §3.4 and §20.
