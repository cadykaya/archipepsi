...
**Copy `schemas/` verbatim.** It runs and it is tested. Run `pytest` on it before writing anything else — 129 tests, all green, both standalone and from the repo root. Do not retype the models from the prose; the prose describes them, the code *is* them. Regenerate `constants.gd` and the JSON Schemas with `python schemas/export.py` and never hand-edit the outputs.
