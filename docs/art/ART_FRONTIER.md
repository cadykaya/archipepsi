...
| 2 | Core interactables | **in progress** — Batch 005 produced the Check as seven files along `reward.gd`'s own node boundaries, including its four states. The portal, `door_standard`, `objective_marker` and `signage_module` remain |
...
| Next action | **Tier 2, continued: the portal.** The Check is produced (Batch 005). `exit_portal.gd` names a `Core` with locked / unlocked states and a remaining-Checks count, and `portal_b2_wound` is still a review model. Then `door_standard` (2.4 × 3.2 m), which every Zone uses. |
...
| `python3 tools/blender/sync_inventory.py` | 62 assets written |
| `tools/check_art_current.sh` | PASS — every asset byte-identical from source |
| Assets built | 62 models + 16 theme textures + 7 prop skins + review images in `review/batch001` … `batch005` |
...
**[`docs/art/review/batch005/`](review/batch005/)** — the current batch
...
for s in materials architecture props concept_epsilon concept_check \
         ...
         batch002_enemies epsilon_installation hub lab check; do
