...
| 3 | Common architecture | **done** — 28 of §3's 31 rows: Batches 001 and 007 the Pri-A modules, 020 the structural Pri-B seven, 021 the services and openings. **All three that remain need an owner or engineering decision, and none is production work.** `arch_signage_mount` and `arch_objective_socket` are blocked with the navigation language; `arch_affordance_socket` turns out to have no placement path either (requirement 22 — corrected here, having been described as merely deferred); `arch_vista_socket` has no contract at all |
...
| 8 | The six theme kits | **6 of 6 material families, dressing (013 `PASS`) and a light fixture family per theme (014 `PASS`, locked as the theme fixture families)**. Landmarks remain |
...
| Phase | **STYLE LOCK PASSED — production.** **Every batch built so far is `PASS`**: 001 through 019, the whole queue cleared 2026-08-28. Nothing is pending. |
| Owner review | Style Lock passed 2026-08-28. Draft PR [#5](https://github.com/cadykaya/archipepsi/pull/5). |
| Next action | **Nothing unblocked remains, and that is now checked rather than assumed.** The last row that looked like ordinary production — `arch_affordance_socket` — has no engine placement path (requirement 22). Everything else of size waits on a decision: the navigation language and its two sockets, the enemy roles' colliders (req 7) and telegraph node (req 14), the theme landmarks (new landmark language, which this lane may not invent), §8's unplaceable prop library, and Tier 9 presentation/polish, which has never been scoped. A heartbeat with nothing productive available says so in one line rather than inventing work — this is that line. |
...
| 22 | **`arch_affordance_socket` has nothing to attach to.** §3 lists it as Pri B and it looks buildable, but `affordance_features.place_all` builds each feature as a COMPLETE node and positions it directly on the chamber floor — the grapple anchor makes its own plate as part of itself, not as a separate mount — and `FOOTPRINT` is a clearance rule consumed by `fits()` and `required_width()`, never geometry. There is no socket node, no mount, and no code path that would place one. | Two questions, and art has answered neither. **Should an affordance sit on an authored mount that reserves and SHOWS its footprint?** That would make `affordance_features`' first rule — *never on the mandatory path, a feature's whole footprint clears it* — visible to the player instead of invisible, which is the one place the seven affordances currently teach nothing about why they are where they are. **Or do features stay floor-placed?** Then the row should be struck from §3 rather than left looking like unbuilt work. Building one now would invent a contract, which is the same reason `arch_objective_socket` is blocked. | `arch_affordance_socket`, and the placement read of every `batch009` asset |
