...
| 7 | Room-shell vocabulary | **done** — 19 shells across all six families: 015 the corridors, 016 the arenas, 017 the platform paths, 018 the towers, 019 the treasure rooms and corners. Every one authored at a discrete size rather than stretched, and every one carrying the semantics the runtime places it by. §7 was empty five batches ago |
...
| Phase | **STYLE LOCK PASSED — production.** Batches 001, 002, 004, 005 (incl. 005-R), 006, 007, 008, 009, 010, 011 and 012 are `PASS`. Batches 003, 013, 014, 015, 016, 017, 018 and 019 are `PENDING`. |
| Owner review | Style Lock passed 2026-08-28. Draft PR [#5](https://github.com/cadykaya/archipepsi/pull/5). |
| Next action | **Architecture Pri-B/C** (Tier 3, 14 modules), which the owner explicitly permitted and which is the largest approved unblocked block left now that Tier 7 is complete. **But** the review queue is eight batches deep and five of them are consecutive shell families — a redirect on shells would cost five batches to redo rather than one. That is worth the owner's attention before another long unreviewed run, and holding is a legitimate heartbeat outcome for exactly this reason. |
...
| Queue depth | **Eight batches pending**: 003 (the Hub, never verdicted on its own), 013, 014, and the five-batch Tier 7 run 015–019. A redirect on shell direction now costs five batches to redo rather than one. Everything else is approved. Holding remains a legitimate heartbeat outcome when the queue grows again, and this line is why. |
...
| `python3 tools/blender/sync_inventory.py` | 121 assets written |
| `tools/check_art_current.sh` | PASS — every asset byte-identical from source |
| Assets built | 121 models + **31 theme textures (six of six families)** + 7 prop skins + review images in `review/batch001` … `batch019` |
...
| 19 | **Which chambers get a ceiling is inconsistent, not absent.** `corridor()`, `corner()` and `treasure_room()` all close their tops — the treasure room explicitly, with a trim-material slab at `height`. `arena()` and `tower()` do not: `_perimeter` gives them a floor and four walls, and `_greeble_room` adds corner buttresses and floor crates and nothing overhead. So it is not a rule that rooms are open; it is that two of the five were never closed — so a corridor chaining into an arena joins a roofed space to an open one at the doorway, and `wall_height`'s 4–8 m range describes walls with nothing on them. Batch 016's arena shells include a ceiling at `wall_height`, which is the only reading that makes the chain continuous. | If open-sky arenas are intended for some themes — `void_glitch` and `temple_ruin` both have a case — that is an engineering decision and the ceiling comes out of the shells; nothing else changes. Art has picked the continuous reading rather than leaving a hole, and is flagging it rather than assuming it. | every `shell_arena_*`, and every room shell after them |
| 20 | **`corner()` paints hazard orange as a navigation marker.** It builds a 0.06 × 1.0 × 2.0 stripe in `ThemeMaterials.hazard_mat` on the inner wall of the turn, purely to say *the corridor bends here*. The owner's Batch 010 ruling is that **orange must remain warning / hazard language** and that generic facility dressing gets its own neutral vocabulary instead — and a turn is not a warning. This is interface requirement 18's problem in a second place: the palette's loudest and most specific colour used because it is available. | Batch 019's authored corners mark the turn with **form** — a chamfered inner corner and a skirting that carries around it — and carry no hazard material at all. If the engine keeps its stripe, a chamber will contradict the asset standing in it, and orange stops meaning anything for the second time. Art has not removed a gameplay cue: the turn is still marked, by geometry. | `shell_corner_left`, `shell_corner_right` |
