...
| ~~`objective_marker`~~ | — | — | `gameplay/zone_controller.gd` | — | **STRUCK 2026-08-29 — OWNER DECISION.** No CURRENT objective type requires a world marker. `reach_reward`: the Check itself is already the world marker. `kill_all`: there is no destination object to mark. `platform_to_goal`: `goal_area_position` `(0, rise + 1.0, total - ledge)` sits ~2 m from `reward_position` `(0, rise, total - ledge / 2.0)` on the same ledge and is crossed naturally on approach to the waypointed Check. A separate marker would duplicate the HUD waypoint, the Check, or both. **Scope: this is NOT a permanent prohibition on world objective markers.** If a future objective introduces a destination that is not represented by an existing world object, not naturally crossed while following the HUD, and genuinely benefits from world-side marking, the need may be revisited deliberately. Until then no speculative asset is kept in the inventory against that possibility. | — |
...
| `signage_module` | L0 | U | `chamber_builders.gd` `GRAFFITI` | B | **PASS 2026-08-29 (Batch 022 / 022-R).** The four-module family `nav_blade`, `nav_panel`, `nav_chevron`, `nav_hanger` is approved in all six themes. Semantic hierarchy: neutral field, glyph/form, placement/orientation, runtime-owned wording. Theme treatment is construction language, never navigation meaning. Direction stays placement/runtime truth and is not baked into the primary sign mesh. |
